...
const AUDIO_FIELDS = 'Genres,AlbumArtist,ArtistItems,UserData';
...
export class JellyfinClient {
  ...
  imageUrl(itemId, type = 'Primary', maxSize = 300, tag = null) {
    ...
    if (this.connectionMode === 'service') {
      ...
      return `${base}/images/${itemId}/${type}?maxSize=${maxSize}&quality=85${tagParam}`;
    }
    const base = this.jellyfinUrl || this.baseUrl;
    const tagParam = tag ? `&tag=${encodeURIComponent(tag)}` : '';
  ...
  getAllAudio(limit = 5000) {
    return this.request(`/Users/${this.userId}/Items`, {
      query: {
        ...
        Recursive: true,
        Limit: limit,
        Fields: `Genres,AlbumArtist,AlbumId`,
      },
    });
  }
  ...
  createPlaylist(name, itemIds = []) {
    // OpenAccess: false → la playlist es privada, solo la ve este usuario.
  ...
  async renamePlaylist(playlistId, newName) {
    ...
    return this.request(`/Playlists/${playlistId}`, {
      method: 'POST',
      body: { Name: newName, Ids: ids, OpenAccess: false, Users: [] },
    });
  }
