...
export default function Login() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    setLoading(true);
    try { await login(serverUrl, username, password, mode); }
      ...
      <div className="login-card">
        ...
        <div className="login-mode">
          <button type="button"
            ...
            onClick={() => setMode('direct')}>Jellyfin</button>
          <button type="button"
            className={`login-mode__btn ${mode === 'service' ? 'login-mode__btn--active' : ''}`}
            onClick={() => setMode('service')}>Aurita Server</button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <label>
            URL del servidor
            <input type="text"
              placeholder={mode === 'direct' ? 'https://jellyfin.midominio.com' : 'https://aurita.midominio.com'}
